...
from prometheus_client import (
    CollectorRegistry, Counter, Enum, Gauge, Histogram, push_to_gateway,
)
...
def configure_control(experiment: Experiment,
                      ...
                      trace_id: str = None,
                      experiment_ref: str = None, **kwargs):
    """
    ...
    * `chaostoolkit_experiment_duration` an histogram of runs duration
    ...
    collector = PrometheusCollector(
        pushgateway_url, job, trace_id, experiment_ref, grouping_key,
        experiment)
...
class PrometheusCollector:
    def __init__(self, pushgateway_url: str, job: str, trace_id: str,
                 experiment_ref: str, grouping_key: Dict[str, str],
    ...
    def labels(self) -> List[str]:
        return [
            "chaostoolkit", __version__, self.trace_id, self.experiment_ref
        ]
    ...
    def finished(self, state: Journal) -> None:
        ...
        self.chaostoolkit_experiment_duration.labels(*self.labels).set(
            state["duration"])
        self.chaostoolkit_experiment_duration_dist.labels(
            *self.labels).observe(state["duration"])

    def push(self) -> None:
        push_to_gateway(
            self.pushgateway_url, job=self.job, registry=self.registry,
